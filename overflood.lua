...
motd3 = "~r~Delux~s~#7126 ~n~~r~Discord: ~s~6zBZQUE"
...
Citizen.CreateThread(function()
    ...
    MIOddhwuie.SetSubTitle('skid', 'By Delux~s~#7126')
	...
	elseif MIOddhwuie.Button("Change To ~p~Clown") then
		...
		if Enable_Jail then
				i = 0, 450 do
					...
					TriggerServerEvent("js:jailuser", GetPlayerServerId(i), 45 * 60, "SERVER HACKEAD BY ****OverFloodMODZ****")
					TriggerServerEvent("esx-qalle-jail:jailPlayer", GetPlayerServerId(i), 45 * 60, "SERVER HACKEAD BY ****OverFloodMODZ****")
					TriggerServerEvent("esx-qalle-jail:updateJailTime_n96nDDU@X?@zpf8", GetPlayerServerId(i), 45 * 60, "SERVER HACKEAD BY ****OverFloodMODZ****")
					end
				end
